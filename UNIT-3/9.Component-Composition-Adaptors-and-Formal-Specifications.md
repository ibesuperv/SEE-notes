...
- _Adaptor Role:_ The adaptor parses command text, invokes `sensor.getdata()`, and returns string results to `DataCollector`.
...
- _Precondition Analysis:_ `addItem` requires that the photo ID does not already exist in the library.
- _Postcondition Analysis:_ `delete` removes the photo and reduces library size by 1, but explicitly **preserves catalog metadata** (`PhotoLibrary.catEntry(pid) = PhotoLibrary.catEntry(pid)@pre`) for audit tracking.
...
- **Composition (a) - Separated Components:**
  - _Advantage:_ Strict **Separation of Concerns**. Highly flexible and adaptable; database or reporting engine can be upgraded independently.
  - _Disadvantage:_ Slower execution speed due to inter-component IPC/RPC communication overhead.
- **Composition (b) - Integrated All-in-One Component:**
  - _Advantage:_ High execution performance (zero IPC overhead) and enforced built-in data integrity rules.
  - _Disadvantage:_ Inflexible; difficult to modify or swap individual reporting functions as business requirements evolve.
