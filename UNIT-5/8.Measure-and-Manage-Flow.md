...
- **Disrupted Flow:** Caused by unevenness (_mura_) and overburdening (_muri_). Work piles up, developers spend time waiting on others.
...
A Cumulative Flow Diagram (CFD) is a graph used in Kanban to monitor workflow, Work in Progress (WIP), lead time, and system stability.
...
- **Core Metrics Tracked:**
  ...
  - **Lead Time ($W$):** how long the user waits. Customer requests Login feature. Gets it after 6 days.
...
- **Unstable System:** Indicated by tilted trendlines (e.g., inventory rising over time).
...
4.  **Class of Service:** Use different colored sticky notes to prioritize:
    - _Pink Stickies:_ Severe cases that cannot be rescheduled (high priority).
    - _Yellow Stickies:_ Minor issues that can flow normally or be rescheduled.
...
- **Why Explicit Policies are Critical:**
  - _Less confusion_
  - _Easier training_
  - _Easier improvement_
  - _Easier to enforce WIP_
