...
_(Based on Ian Sommerville, "Software Engineering" - Chapter 4)_
...
- _Practical Approach:_ A mix of both (starting with specific questions that lead into open discussion).
...
Ethnography is an observational technique in which the analyst works alongside users and observes their day-to-day activities.

#### Key Strengths of Ethnography:

1.  **Discovers Implicit Requirements:** Uncovers how people _actually_ work rather than how official corporate procedures say they _ought_ to work.
    - _Real-World Example:_ Air Traffic Controllers (ATCs) often turn off automated conflict alerts because they issue annoying false alarms during routine flight paths, relying instead on visual awareness of adjacent controller sectors.
...
- _Initial Assumption:_ Teacher logged into photo site with images stored locally on a tablet.
- _Normal Flow:_ Selects photos, assigns project name, adds keywords, clicks upload. System appends username to filename and emails moderator.
- _What Can Go Wrong:_ No moderator exists $\rightarrow$ system auto-generates email to administrator to assign one and warns the user of delay. Duplicate file exists $\rightarrow$ prompts to overwrite, rename, or cancel.
- _Final State:_ Selected photos uploaded and tagged as "awaiting moderation."
